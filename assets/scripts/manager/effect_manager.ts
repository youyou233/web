...
export default class EffectManager extends cc.Component {
    ...
    createLabel(string: string, pos: cc.Vec2, pox: number = 50) {
        let label = PoolManager.instance.createObjectByName('damageLabel', GameUI.instance.view.nodeLabelContainer)
    ...
    createMoneyEffect(num: number, start: cc.Vec2, end: cc.Vec2) {
    ...
    //     if (!parent) {
    //         parent = LoopUIManager.instance.spnodeEffectContainer
    //     }
    //     let name = luck ? 'luckItem' : 'spItem'
    //     let sp = PoolManager.instance.createObjectByName(name, parent)
    //     LoopUIManager.instance.spnodeEffectContainer.zIndex = zindex
    //     sp.angle = 0
    //     sp.scale = 1
    //     sp.opacity = 255
    //     sp.stopAllActions()
    ...
    //     //向四周散开之后进入背包
    // }
    createEffect(name: string, pos: cc.Vec3 | cc.Vec2, node: cc.Node = GameUI.instance.view.nodeEffectContainer) {
        if (!name) return
    ...
    //     let sp = PoolManager.instance.createObjectByName('spItem', LoopUIManager.instance.spnodeEffectContainer)
    //     LoopUIManager.instance.spnodeEffectContainer.zIndex = 199
    //     sp.angle = 0
    //     sp.scale = 1
    //     sp.opacity = 255
    //     sp.stopAllActions()
    ...
    createPartical(name: string, pos: cc.Vec2, node: cc.Node = GameUI.instance.view.nodeEffectContainer) {
    ...
    createParticalMoveEffect(par: string, start: cc.Vec2, end: cc.Vec2, cb, container = GameUI.instance.view.nodeEffectContainer) {
        let node = PoolManager.instance.createObjectByName("itemPartical", container)
        node.getComponent(ItemPartical).init(par, start, end, cb)
    }
